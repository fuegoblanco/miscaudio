...
export const loadScene_music = async function (options = {}) {
  const {
    onProgress,
    domElement,
    configureSceneData,
  ...
  const promises = [
    ...
    sceneData == null
      ? loadSceneData_music({ onProgress, sceneDataRoot })
      : (() => {
          return new Promise((resolve) => resolve());
        })(),
  ];
  const results = await Promise.all(promises);
  const { Poly, loadSceneFromSceneData_music } = results[0];
  if (sceneData == null) {
    sceneData = results[1];
  }
  ...
  const loadedData = await loadSceneFromSceneData_music({
    onProgress,
    sceneData,
    domElement,
    runRegister,
